...
var BARCHARTTRANSLATEY = 180;
var TSTRANSLATEX = 24;
var TSTRANSLATEY = 330;
...
var init = function() {
    // create svg panels
    svg = d3.select('#juxtalearn-map').append('svg');  
...
var createUi = function() {
    ...
    var ui = (new Element('div', {
        'id': 'ui'
    })).inject(document.getElement('#juxtalearn-map'));
    ...
    choiceNegative.grab((new Element('span', {'text':'Activity'})), 'after');
    ...
    choicePositive.grab((new Element('span', {'text':'Problem'})), 'after');
...
var selectCountry = function(id, showTooltip){
    ...
    if(showTooltip){
        var tooltip = d3.select('#juxtalearn-map').append('div')
            .attr('class', 'tooltip')
            .html('<strong>' + data.name + '</strong><br/>Activity: ' + data.negative + '<br/>Problem: ' + data.positive)
		...
		if(graph.links.length > 0){ 		
			...
			.append("svg:a")
			  /*.attr("xlink:href", function(d) { 
			  					var ev = (d.source.type == 'trickytopic') ? '' : '';
								return d.source.url+ev+'/'+d.target.id; })
			  .attr("class", "linkpath") */
...
var addTitle = function(){
	d3.xml(MyAjax.pluginurl+'images/juxtalearn-logo.svg', "image/svg+xml", function(xml) {  
	  var importedNode = importNode(xml.documentElement, true);
	  var logo = svg
		.append("g")
		.attr('id', 'maplogo')
		.attr('transform', 'scale(' + [0.4,0.4] + ')translate(' + [24, 14] + ')');
...
var displayMarkers = function(data){
	...
	marker.enter()
		...
		.on('mouseover', function(d){
			d3.select('#juxtalearn-map').append('div')
            .attr('class', 'tooltip')
