...
new JuxtaLearn_Hub_Shortcode_Example_GeoMap();
class JuxtaLearn_Hub_Shortcode_Example_GeoMap extends JuxtaLearn_Hub_Shortcode {
	var $shortcode = 'example_geomap';
	var $defaults = array(
		'post_id' => false,
		'post_ids' => false,
		'title' => false,
	...
	function content() {
		...
		$sub_options = array_merge($sub_options, array(
			'polarity' => array(
				...
				'save_as' => 'term',
				'label' => "Polarity",
				'options' => get_terms('juxtalearn_hub_polarity', 'hide_empty=0&orderby=id'),
				),
		));
		 $sub_options = array_merge($sub_options, array(
			'sector' => array(
        ...
        <![endif]-->
		 ...
		 /* <![CDATA[ */	
		 	var json = <?php print_r(file_get_contents(site_url().'/'.get_option('json_api_base', 'api').'/hub/get_geojson/?count=-1&type=example,location')); ?>;	
