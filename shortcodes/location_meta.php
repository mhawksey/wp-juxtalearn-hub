...
new JuxtaLearn_Hub_Shortcode_Location_Meta();
class JuxtaLearn_Hub_Shortcode_Location_Meta extends JuxtaLearn_Hub_Shortcode {
	var $shortcode = 'location_meta';
	var $defaults = array(
		'post_id' => false,
		'post_ids' => false,
		'title' => false,
		...
		'no_example_message' => "There is no meta data for this location",
		'title_tag' => 'h4',
	);

	

	static $post_types_with_example = array('location');
	
	function add_to_page($content) {
		if (in_array(get_post_type(), self::$post_types_with_example)) {
			$content = (($this->defaults['header_terms']) ? do_shortcode('[location_meta location="header"]') : '').$content.(($this->defaults['footer_terms']) ? do_shortcode('[location_meta location="footer"]') : '');
		}
		return $content;
	}
